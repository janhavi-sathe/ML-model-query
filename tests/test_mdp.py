...
from TMM.algs import value_iteration, policy_iteration
from aicoach.algs.IRL.soft_value_iteration import soft_value_iteration
import aicoach.algs.utils.test_utils as test_utils
...
class FrozenLakeMDP(mdp_lib.MDP):
  ...
  def init_statespace(self):
    ...
    self.s_space = mdp_lib.StateSpace(statespace=list_state)
    self.dict_factored_statespace = {0: self.s_space}
    self.dummy_states = mdp_lib.StateSpace(statespace=["terminal"])

  def init_actionspace(self):

    self.dict_factored_actionspace = {}
    self.a_space = mdp_lib.ActionSpace(actionspace=[
...
def test_soft_value_iteration_toy():
  ...
  gamma = 0.9
  np_v_value, np_q_value = soft_value_iteration(toy_mdp.np_transition_model,
                                                toy_mdp.np_reward_model,
                                                discount_factor=gamma,
                                                max_iteration=100,
                                                epsilon=0.001)
...
def test_policy_iteration_toy():
  ...
  pi, np_v_value, np_q_value = policy_iteration(
...
if __name__ == "__main__":
  ...
  pi, np_v_value, np_q_value = value_iteration(
  ...
  np_v_value, np_q_value = soft_value_iteration(
      toy_mdp_large_reward.np_transition_model,
      toy_mdp_large_reward.np_reward_model,
      discount_factor=gamma,
      max_iteration=100,
