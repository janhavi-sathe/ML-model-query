...
def query():
    ...
    if request.method == "POST":
        ...
        per_page = 8  # Modified to display 8 texts per page
    else:
    ...
    try:
        ...
        if category not in range(0, 6):  # Limit 0~5
...
def find_similar_images():
    try:
        ...
        base_path = os.path.dirname(os.path.abspath(__file__))
        sim_mat_path = os.path.join(base_path, "..", "text_sim_matrix.npy")
        ...
        similar_texts = text_data_df.loc[sorted_indices, ['text', 'main_words', 'pred_labels', 'prediction']]
        ...
        return jsonify({"error": "text_sim_matrix.npy not found"}), 500
...
def find_keyword():
    try:
        ...
        keyword_data = text_data_df.loc[text_data_df['text'].astype(str).str.contains(keyword, na=False), ['text', 'main_words', 'pred_labels', 'prediction']]
