...
from .nurse_mdp import THONursePolicy
import numpy as np
  ...
  def save_history(self, file_name, header):
    ...
    with open(file_name, 'w', newline='') as txtfile:
      ...
      for tup_state, tup_actions, cur_latent in self.history:
        state_idx = self.mmdp.conv_sim_states_to_mdp_sidx(tup_state)
        action_idx = self.mmdp.conv_sim_actions_to_mdp_aidx(tup_actions)
        latent_idx = self.nurse_agent.policy_model.conv_latent_to_idx(
            cur_latent)
    ...
    with open(os.path.join(DATA_DIR, "readable", os.path.basename(file_name)),
              'w',
              newline='') as txtfile:
      # sequence
      txtfile.write(header)
      txtfile.write('\n')
      txtfile.write(
          'patient_vital, nurse_dir, nurse_pos, nurse_tool, surgeon_tool, surgeon_ready, perf_ready, anes_ready, cur_step, cur_requirement, nurse_asked; tup_actions; cur_latent;\n'
      )
