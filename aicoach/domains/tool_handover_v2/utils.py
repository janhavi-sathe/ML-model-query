...
from .mdp import MDP_ToolHandover_V2
from .simulator import ToolHandoverV2Simulator
import numpy as np
from aicoach.algs.utils.data_utils import Trajectories
from TMM.domains.rescue_v2.define import AGENT_ACTIONSPACE
from TMM.domains.rescue_v2.simulator import RescueSimulatorV2
from TMM.domains.rescue_v2.mdp import MDP_Rescue_Task

from .define import NURSE_ACTIONSPACE
from TMM.domains.rescue_v2.simulator import RescueSimulatorV2
from TMM.domains.rescue_v2.mdp import MDP_Rescue_Task
  ...
  def load_from_files(self, file_names):
    for file_nm in file_names:
      ...
      for tidx, vec_state_action in enumerate(trj):
        ...
        aidxn = NURSE_ACTIONSPACE.action_to_idx[
            actions[0]] if actions[0] is not None else Trajectories.EPISODE_END
        aidxs = self.task_mdp.dict_factored_actionspace[1].action_to_idx[
            actions[1]] if actions[1] is not None else Trajectories.EPISODE_END
        aidxa = self.task_mdp.dict_factored_actionspace[2].action_to_idx[
            actions[2]] if actions[2] is not None else Trajectories.EPISODE_END
        aidxp = self.task_mdp.dict_factored_actionspace[3].action_to_idx[
            actions[3]] if actions[3] is not None else Trajectories.EPISODE_END
